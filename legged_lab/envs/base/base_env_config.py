...
class BaseEnvCfg:
    ...
    commands: CommandsCfg = CommandsCfg(
        ...
        rel_heading_envs=0.0,  # 默认不使用heading
        heading_command=False,  # 默认关闭heading
        heading_control_stiffness=0.5,
        debug_vis=True,
        ranges=CommandRangesCfg(
            lin_vel_x=(0.5, 1.5),  # 修改：强制前进，不包括0
            lin_vel_y=(-0.3, 0.3),  # 修改：减小侧向范围
            ang_vel_z=(-1.0, 1.0),  # 保持原样
    ...
    domain_rand: DomainRandCfg = DomainRandCfg(
        events=EventCfg(
            ...
            reset_base=EventTerm(
                ...
                mode="reset",
                params={
                    "pose_range": {"x": (-0.5, 0.5), "y": (-0.5, 0.5), "yaw": (-0.3, 0.3)},  # 限制yaw在±17°
                    "velocity_range": {
                        "x": (-0.5, 0.5),
                        "y": (-0.5, 0.5),
                        "z": (-0.5, 0.5),
...
class BaseAgentCfg(RslRlOnPolicyRunnerCfg):
    seed = 42
    device = "cuda:0"
    num_steps_per_env = 256  # 从128增加到256，进一步延长rollout学习更长期行为
    ...
    algorithm = RslRlPpoAlgorithmCfg(
        ...
        num_mini_batches=4,
        learning_rate=2.0e-3,  # 增加到2e-3，加快学习速度
        schedule="adaptive",
        gamma=0.99,
        lam=0.95,
        desired_kl=0.01,
